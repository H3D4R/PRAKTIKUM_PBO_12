# [INF2143] Proyek SOLID OOP Python

## Pertemuan 12: Dokumentasi dan Version Control

### Deskripsi Proyek

Proyek ini mendemostrasikan implementasi prinsip SOLID (SRP, DCP, DIP) pada sistem checkout dengan mengggunakan Abstraction (Interface) dan Dependency Injection.

### Struktur File

- `refactor_solid.py` : Kode inti yang sudah direfaktor dan ditambahkan logging
- `README.md` : Dokumen ini.

### Cara Menjalankan

1. Pastikan Python 3.x terinstal.
2. Jalankan file utama dari terminal
   ```bash
   python refactor_solid.py
   ```

### Histori Perubahan (log)

Kode dikelola menggunakan Git.
